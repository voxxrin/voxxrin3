import type {TalkMatchingYoutubeTestData} from "./test-data";

export const VDZ24_TALKS_AND_YOUTUBE = {
  "expectedMappedTalks": [
    {
      "__score": 0.639,
      "__talkTitle": "Welcome",
      "__speakers": "Federico Yankelevich, Patrick Baumgartner",
      "__videoTitle": "Welcome to the 8th edition of VoxxedDays Zürich by Federico Yankelevich and Patrick Baumgartner",
      "talkId": "11653",
      "videoId": "N7xoZbxqKhE"
    },
    {
      "__score": 0.819,
      "__talkTitle": "Avoid mastering Kubernetes for local development with Dapr and Testcontainers.",
      "__speakers": "Mauricio Salatino, Oleg Šelajev",
      "__videoTitle": "Avoid mastering Kubernetes for local dev. with Dapr and Testcontainers by M. Salatino and O. Šelajev",
      "talkId": "6575",
      "videoId": "VYCelta9WdQ"
    },
    {
      "__score": 0.872,
      "__talkTitle": "NLP à la Suisse",
      "__speakers": "Andrea Mocci, Jesper Findahl",
      "__videoTitle": "NLP à la Suisse by Andrea Mocci and Jesper Findahl",
      "talkId": "2934",
      "videoId": "kxYhulu7mmI"
    },
    {
      "__score": 0.898,
      "__talkTitle": "Hitting the “refresh” button on your Spring web application",
      "__speakers": "Stéphane Nicoll, Brian Clozel",
      "__videoTitle": "Hitting the “refresh” button on your Spring web application by Stéphane Nicoll",
      "talkId": "6570",
      "videoId": "8vtoa8VC-gY"
    },
    {
      "__score": 0.919,
      "__talkTitle": "A fun and absurd introduction to Vector Databases",
      "__speakers": "Alexander Chatzizacharias",
      "__videoTitle": "A fun and absurd introduction to Vector Databases by Alex Chatzizacharias",
      "talkId": "3865",
      "videoId": "Kw41RfTWyQ4"
    },
    {
      "__score": 0.92,
      "__talkTitle": "Using AI in Software Design: How ChatGPT Can Help With Creating a Solution Architecture",
      "__speakers": "Ralf D. Müller",
      "__videoTitle": "Using AI in Software Design: How ChatGPT Can Help With Creating a Solution Arch. by Ralf D. Müller",
      "talkId": "7666",
      "videoId": "Fv7hpwUgXz8"
    },
    {
      "__score": 0.928,
      "__talkTitle": "State of change detection in Angular and how far are we from Zoneless applications ? ",
      "__speakers": "Matthieu Riegler",
      "__videoTitle": "State of change detection in Angular and how far are we from Zoneless applications ? by M. Riegler",
      "talkId": "6560",
      "videoId": "KdK20Cn9Y4M"
    },
    {
      "__score": 0.933,
      "__talkTitle": "Modern Java in Action",
      "__speakers": "Nicolai Parlog",
      "__videoTitle": "Modern Java in Action by Nicolai Parlog",
      "talkId": "2926",
      "videoId": "Pmlpk4Ti9f8"
    },
    {
      "__score": 0.937,
      "__talkTitle": "The dark side of kotlin",
      "__speakers": "Jarek Ratajski",
      "__videoTitle": "The dark side of kotlin by Jarek Ratajski",
      "talkId": "7182",
      "videoId": "25R3WMNSqho"
    },
    {
      "__score": 0.941,
      "__talkTitle": "IntelliJ IDEA Tips & Tricks",
      "__speakers": "Anton Arhipov",
      "__videoTitle": "IntelliJ IDEA Tips & Tricks by Anton Arhipov",
      "talkId": "2375",
      "videoId": "53ccfLqYpWY"
    },
    {
      "__score": 0.945,
      "__talkTitle": "From Java 17 to 21: A Showcase of JDK Security Enhancements",
      "__speakers": "Ana-Maria Mihalceanu",
      "__videoTitle": "From Java 17 to 21: A Showcase of JDK Security Enhancements by Ana Maria Mihalceanu",
      "talkId": "2930",
      "videoId": "tQxPRm6-PA8"
    },
    {
      "__score": 0.945,
      "__talkTitle": "Practical Performance Analysis",
      "__speakers": "Simone Bordet",
      "__videoTitle": "Practical Performance Analysis by Simone Bordet",
      "talkId": "2949",
      "videoId": "tn1Q7-dAHd8"
    },
    {
      "__score": 0.947,
      "__talkTitle": "Principles of Fluent API Design",
      "__speakers": "David Beaumont",
      "__videoTitle": "Principles of Fluent API Design by David Beaumont",
      "talkId": "7181",
      "videoId": "VPu-ytfYTeU"
    },
    {
      "__score": 0.949,
      "__talkTitle": "Power structures. The fair advantage",
      "__speakers": "Anja Kunkel",
      "__videoTitle": "Power structures. The fair advantage by Anja Kunkel",
      "talkId": "1787",
      "videoId": "p6GUmeOIBUg"
    },
    {
      "__score": 0.951,
      "__talkTitle": "ML in Java, YES it's possible!",
      "__speakers": "Mohammed Aboullaite",
      "__videoTitle": "ML in Java, YES it's possible! by Mohammed Aboullaite",
      "talkId": "1776",
      "videoId": "lPYmDMOJVuM"
    },
    {
      "__score": 0.951,
      "__talkTitle": "The Hidden Dynamic Life of Java",
      "__speakers": "Nataliia Dziubenko",
      "__videoTitle": "The Hidden Dynamic Life of Java by Nataliia Dziubenko",
      "talkId": "2935",
      "videoId": "tkZNBFLti1w"
    },
    {
      "__score": 0.958,
      "__talkTitle": "Neurospicy: ADHD as a Competitive Advantage",
      "__speakers": "Myriam Jessier",
      "__videoTitle": "Neurospicy: ADHD as a Competitive Advantage by Myriam Jessier",
      "talkId": "4312",
      "videoId": "kbPxRhkYmjU"
    },
    {
      "__score": 0.958,
      "__talkTitle": "Observability 101 with Spring and Micrometer",
      "__speakers": "Nele Uhlemann",
      "__videoTitle": "Observability 101 with Spring and Micrometer by Nele Uhlemann",
      "talkId": "6551",
      "videoId": "Ssje93u2GWM"
    },
    {
      "__score": 0.959,
      "__talkTitle": "Faster or better designed? Choose any two!",
      "__speakers": "Dmytro Vyazelenko",
      "__videoTitle": "Faster or better designed? Choose any two! by Dmytro Vyazelenko",
      "talkId": "2368",
      "videoId": "pdFqKiwgZZY"
    },
    {
      "__score": 0.959,
      "__talkTitle": "Automate your Changelogs with Release Drafter",
      "__speakers": "Oleg Nenashev",
      "__videoTitle": "Automate your Changelogs with Release Drafter by Oleg Nenashev",
      "talkId": "4327",
      "videoId": "-PIkiXAg8ws"
    },
    {
      "__score": 0.96,
      "__talkTitle": "Who is Jason, and what’s he doing in my database?",
      "__speakers": "Martin Bach",
      "__videoTitle": "Who is Jason, and what’s he doing in my database? by Martin Bach",
      "talkId": "1790",
      "videoId": "pfjAnRl1M0A"
    },
    {
      "__score": 0.961,
      "__talkTitle": "SQL Test Kit, or how to unit test any SQL query",
      "__speakers": "Victor Landeau",
      "__videoTitle": "SQL Test Kit, or how to unit test any SQL query by Victor Landeau",
      "talkId": "7156",
      "videoId": "chB5f80U9jA"
    },
    {
      "__score": 0.962,
      "__talkTitle": "Password-less apps: implementing WebAuthN",
      "__speakers": "Daniel Garnier-Moiroux",
      "__videoTitle": "Password-less apps: implementing WebAuthN by Daniel Garnier-Moiroux",
      "talkId": "2355",
      "videoId": "FUWLYC1z1LU"
    },
    {
      "__score": 0.962,
      "__talkTitle": "Performance oriented Spring Data JPA & Hibernate",
      "__speakers": "Maciej Walkowiak",
      "__videoTitle": "Performance oriented Spring Data JPA & Hibernate by Maciej Walkowiak",
      "talkId": "7670",
      "videoId": "exqfB1WaqIw"
    },
    {
      "__score": 0.963,
      "__talkTitle": "Securing the Supply Chain for Your Java Applications",
      "__speakers": "Thomas Vitale",
      "__videoTitle": "Securing the Supply Chain for Your Java Applications by Thomas Vitale",
      "talkId": "2927",
      "videoId": "pS4cmqNN1QY"
    },
    {
      "__score": 0.965,
      "__talkTitle": "React Server Components - Explained for Backend Developers",
      "__speakers": "Jonas Bandi",
      "__videoTitle": "React Server Components - Explained for Backend Developers by Jonas Bandi",
      "talkId": "7190",
      "videoId": "ZRRygeUhSVU"
    },
    {
      "__score": 0.967,
      "__talkTitle": "Navigating the Chaos: A Holistic Approach to Incident Management",
      "__speakers": "Hila Fish",
      "__videoTitle": "Navigating the Chaos: A Holistic Approach to Incident Management by Hila Fish",
      "talkId": "2941",
      "videoId": "WEviDS-8pO4"
    },
    {
      "__score": 0.967,
      "__talkTitle": "From k9s to OpenTelemetry: A guide to observability for your apps in K8s",
      "__speakers": "Matthias Haeussler",
      "__videoTitle": "From k9s to OpenTelemetry: A guide to observability for your apps in K8s by Matthias Haeussle",
      "talkId": "7163",
      "videoId": "piM7PEswmso"
    },
    {
      "__score": 0.968,
      "__talkTitle": "A Deep Dive Into Advanced TypeScript: A Live-Coding Expedition",
      "__speakers": "Christian Wörz",
      "__videoTitle": "A Deep Dive Into Advanced TypeScript: A Live-Coding Expedition by Christian Wörz",
      "talkId": "2367",
      "videoId": "zpUQZFrRGHY"
    },
    {
      "__score": 0.969,
      "__talkTitle": "Instrument to Remove: Using Java agents for fun and profit",
      "__speakers": "Johannes Bechberger",
      "__videoTitle": "Instrument to Remove: Using Java agents for fun and profit by Johannes Bechberger",
      "talkId": "1754",
      "videoId": "JnJgvcZo7b8"
    },
    {
      "__score": 0.969,
      "__talkTitle": "The Art and Science of Thriving Developers: 7 Key Strategies",
      "__speakers": "Dimitris Andreadis",
      "__videoTitle": "The Art and Science of Thriving Developers: 7 Key Strategies by Dimitris Andreadis",
      "talkId": "4992",
      "videoId": "IBUBm0WyZW8"
    },
    {
      "__score": 0.97,
      "__talkTitle": "Mastering complex reactivity with template-driven forms and Signals",
      "__speakers": "Brecht Billiet",
      "__videoTitle": "Mastering complex reactivity with template-driven forms and Signals by Brecht Billiet",
      "talkId": "2373",
      "videoId": "2PGKQHiGyio"
    },
    {
      "__score": 0.97,
      "__talkTitle": "Back to the future - how we integrated a 40 year old software system",
      "__speakers": "Lukas Pradel",
      "__videoTitle": "Back to the future - how we integrated a 40 year old software system by Lukas Pradel",
      "talkId": "3857",
      "videoId": "oMqqJuJt0PI"
    },
    {
      "__score": 0.971,
      "__talkTitle": "Faster, greener, and happier- why Quarkus should be your next tech stack",
      "__speakers": "Holly Cummins",
      "__videoTitle": "Faster, greener, and happier- why Quarkus should be your next tech stack by Holly Cummins",
      "talkId": "6556",
      "videoId": "jMWXzKZCKqA"
    },
    {
      "__score": 0.973,
      "__talkTitle": "Communication for Developers: estimates, code reviews, and other hard problems",
      "__speakers": "Voytek Pituła",
      "__videoTitle": "Communication for Developers: estimates, code reviews, and other hard problems by Voytek Pituła",
      "talkId": "4983",
      "videoId": "bVRbxVCHrPE"
    },
    {
      "__score": 0.974,
      "__talkTitle": "The Code Quality Advantage: How Empirical Data Shatters the Speed vs Quality Myth",
      "__speakers": "Adam Tornhill",
      "__videoTitle": "The Code Quality Advantage: How Empirical Data Shatters the Speed vs Quality Myth by Adam Tornhill",
      "talkId": "11651",
      "videoId": "ty9d7wEVTwc"
    }
  ],
  "expectedUnmappedTalks": [
    {
      "__talkTitle": "Speed up your development practices: Decouple application from infrastructure",
      "__talkFormat": "Conference (id=953, duration=PT45m)",
      "__talkRoom": "Room 7 (4302)",
      "__talkSpeakers": "Mauricio Salatino",
      "talkId": "4995"
    }
  ],
  "unmappedYoutubeVideos": [],
  "youtubeVideos": [
    {
      "id": "piM7PEswmso",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT44M32S",
      "title": "From k9s to OpenTelemetry: A guide to observability for your apps in K8s by Matthias Haeussle"
    },
    {
      "id": "tQxPRm6-PA8",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT45M11S",
      "title": "From Java 17 to 21: A Showcase of JDK Security Enhancements by Ana Maria Mihalceanu"
    },
    {
      "id": "tkZNBFLti1w",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT17M49S",
      "title": "The Hidden Dynamic Life of Java by Nataliia Dziubenko"
    },
    {
      "id": "tn1Q7-dAHd8",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT37M44S",
      "title": "Practical Performance Analysis by Simone Bordet"
    },
    {
      "id": "ty9d7wEVTwc",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT36M28S",
      "title": "The Code Quality Advantage: How Empirical Data Shatters the Speed vs Quality Myth by Adam Tornhill"
    },
    {
      "id": "zpUQZFrRGHY",
      "publishedAt": "2024-03-13T20:50:35Z",
      "duration": "PT44M27S",
      "title": "A Deep Dive Into Advanced TypeScript: A Live-Coding Expedition by Christian Wörz"
    },
    {
      "id": "exqfB1WaqIw",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT43M44S",
      "title": "Performance oriented Spring Data JPA & Hibernate by Maciej Walkowiak"
    },
    {
      "id": "jMWXzKZCKqA",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT45M12S",
      "title": "Faster, greener, and happier- why Quarkus should be your next tech stack by Holly Cummins"
    },
    {
      "id": "kbPxRhkYmjU",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT19M18S",
      "title": "Neurospicy: ADHD as a Competitive Advantage by Myriam Jessier"
    },
    {
      "id": "kxYhulu7mmI",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT44M48S",
      "title": "NLP à la Suisse by Andrea Mocci and Jesper Findahl"
    },
    {
      "id": "lPYmDMOJVuM",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT46M42S",
      "title": "ML in Java, YES it's possible! by Mohammed Aboullaite"
    },
    {
      "id": "oMqqJuJt0PI",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT46M",
      "title": "Back to the future - how we integrated a 40 year old software system by Lukas Pradel"
    },
    {
      "id": "p6GUmeOIBUg",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT36M20S",
      "title": "Power structures. The fair advantage by Anja Kunkel"
    },
    {
      "id": "pS4cmqNN1QY",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT44M26S",
      "title": "Securing the Supply Chain for Your Java Applications by Thomas Vitale"
    },
    {
      "id": "pdFqKiwgZZY",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT44M59S",
      "title": "Faster or better designed? Choose any two! by Dmytro Vyazelenko"
    },
    {
      "id": "pfjAnRl1M0A",
      "publishedAt": "2024-03-13T20:50:33Z",
      "duration": "PT43M51S",
      "title": "Who is Jason, and what’s he doing in my database? by Martin Bach"
    },
    {
      "id": "Kw41RfTWyQ4",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT45M47S",
      "title": "A fun and absurd introduction to Vector Databases by Alex Chatzizacharias"
    },
    {
      "id": "N7xoZbxqKhE",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT16M24S",
      "title": "Welcome to the 8th edition of VoxxedDays Zürich by Federico Yankelevich and Patrick Baumgartner"
    },
    {
      "id": "Pmlpk4Ti9f8",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT43M37S",
      "title": "Modern Java in Action by Nicolai Parlog"
    },
    {
      "id": "Ssje93u2GWM",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT38M6S",
      "title": "Observability 101 with Spring and Micrometer by Nele Uhlemann"
    },
    {
      "id": "VPu-ytfYTeU",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT19M2S",
      "title": "Principles of Fluent API Design by David Beaumont"
    },
    {
      "id": "VYCelta9WdQ",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT46M15S",
      "title": "Avoid mastering Kubernetes for local dev. with Dapr and Testcontainers by M. Salatino and O. Šelajev"
    },
    {
      "id": "WEviDS-8pO4",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT38M45S",
      "title": "Navigating the Chaos: A Holistic Approach to Incident Management by Hila Fish"
    },
    {
      "id": "ZRRygeUhSVU",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT54M26S",
      "title": "React Server Components - Explained for Backend Developers by Jonas Bandi"
    },
    {
      "id": "bVRbxVCHrPE",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT47M2S",
      "title": "Communication for Developers: estimates, code reviews, and other hard problems by Voytek Pituła"
    },
    {
      "id": "chB5f80U9jA",
      "publishedAt": "2024-03-13T20:50:32Z",
      "duration": "PT13M7S",
      "title": "SQL Test Kit, or how to unit test any SQL query by Victor Landeau"
    },
    {
      "id": "-PIkiXAg8ws",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT18M57S",
      "title": "Automate your Changelogs with Release Drafter by Oleg Nenashev"
    },
    {
      "id": "25R3WMNSqho",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT46M8S",
      "title": "The dark side of kotlin by Jarek Ratajski"
    },
    {
      "id": "2PGKQHiGyio",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT43M49S",
      "title": "Mastering complex reactivity with template-driven forms and Signals by Brecht Billiet"
    },
    {
      "id": "53ccfLqYpWY",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT46M41S",
      "title": "IntelliJ IDEA Tips & Tricks by Anton Arhipov"
    },
    {
      "id": "8vtoa8VC-gY",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT47M35S",
      "title": "Hitting the “refresh” button on your Spring web application by Stéphane Nicoll"
    },
    {
      "id": "FUWLYC1z1LU",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT45M45S",
      "title": "Password-less apps: implementing WebAuthN by Daniel Garnier-Moiroux"
    },
    {
      "id": "Fv7hpwUgXz8",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT46M28S",
      "title": "Using AI in Software Design: How ChatGPT Can Help With Creating a Solution Arch. by Ralf D. Müller"
    },
    {
      "id": "IBUBm0WyZW8",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT17M35S",
      "title": "The Art and Science of Thriving Developers: 7 Key Strategies by Dimitris Andreadis"
    },
    {
      "id": "JnJgvcZo7b8",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT40M33S",
      "title": "Instrument to Remove: Using Java agents for fun and profit by Johannes Bechberger"
    },
    {
      "id": "KdK20Cn9Y4M",
      "publishedAt": "2024-03-13T20:50:31Z",
      "duration": "PT40M52S",
      "title": "State of change detection in Angular and how far are we from Zoneless applications ? by M. Riegler"
    }
  ],
  "talks": [
    {
      "id": "11651",
      "title": "The Code Quality Advantage: How Empirical Data Shatters the Speed vs Quality Myth",
      "speakers": [
        {
          "fullName": "Adam Tornhill"
        }
      ],
      "format": {
        "duration": "PT40m",
        "id": "11002",
        "title": "Keynote 40'"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "11653",
      "title": "Welcome",
      "speakers": [
        {
          "fullName": "Federico Yankelevich"
        },
        {
          "fullName": "Patrick Baumgartner"
        }
      ],
      "format": {
        "duration": "PT10m",
        "id": "1002",
        "title": "Welcome"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "1754",
      "title": "Instrument to Remove: Using Java agents for fun and profit",
      "speakers": [
        {
          "fullName": "Johannes Bechberger"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "1776",
      "title": "ML in Java, YES it's possible!",
      "speakers": [
        {
          "fullName": "Mohammed Aboullaite"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "1787",
      "title": "Power structures. The fair advantage",
      "speakers": [
        {
          "fullName": "Anja Kunkel"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "1790",
      "title": "Who is Jason, and what’s he doing in my database?",
      "speakers": [
        {
          "fullName": "Martin Bach"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "2355",
      "title": "Password-less apps: implementing WebAuthN",
      "speakers": [
        {
          "fullName": "Daniel Garnier-Moiroux"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "2367",
      "title": "A Deep Dive Into Advanced TypeScript: A Live-Coding Expedition",
      "speakers": [
        {
          "fullName": "Christian Wörz"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "2368",
      "title": "Faster or better designed? Choose any two!",
      "speakers": [
        {
          "fullName": "Dmytro Vyazelenko"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "2373",
      "title": "Mastering complex reactivity with template-driven forms and Signals",
      "speakers": [
        {
          "fullName": "Brecht Billiet"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "2375",
      "title": "IntelliJ IDEA Tips & Tricks",
      "speakers": [
        {
          "fullName": "Anton Arhipov"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "2926",
      "title": "Modern Java in Action",
      "speakers": [
        {
          "fullName": "Nicolai Parlog"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "2927",
      "title": "Securing the Supply Chain for Your Java Applications",
      "speakers": [
        {
          "fullName": "Thomas Vitale"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "2930",
      "title": "From Java 17 to 21: A Showcase of JDK Security Enhancements",
      "speakers": [
        {
          "fullName": "Ana-Maria Mihalceanu"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "2934",
      "title": "NLP à la Suisse",
      "speakers": [
        {
          "fullName": "Andrea Mocci"
        },
        {
          "fullName": "Jesper Findahl"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "2935",
      "title": "The Hidden Dynamic Life of Java",
      "speakers": [
        {
          "fullName": "Nataliia Dziubenko"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "2941",
      "title": "Navigating the Chaos: A Holistic Approach to Incident Management",
      "speakers": [
        {
          "fullName": "Hila Fish"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "2949",
      "title": "Practical Performance Analysis",
      "speakers": [
        {
          "fullName": "Simone Bordet"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "3857",
      "title": "Back to the future - how we integrated a 40 year old software system",
      "speakers": [
        {
          "fullName": "Lukas Pradel"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "3865",
      "title": "A fun and absurd introduction to Vector Databases",
      "speakers": [
        {
          "fullName": "Alexander Chatzizacharias"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "4312",
      "title": "Neurospicy: ADHD as a Competitive Advantage",
      "speakers": [
        {
          "fullName": "Myriam Jessier"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "4327",
      "title": "Automate your Changelogs with Release Drafter",
      "speakers": [
        {
          "fullName": "Oleg Nenashev"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "4983",
      "title": "Communication for Developers: estimates, code reviews, and other hard problems",
      "speakers": [
        {
          "fullName": "Voytek Pituła"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "4992",
      "title": "The Art and Science of Thriving Developers: 7 Key Strategies",
      "speakers": [
        {
          "fullName": "Dimitris Andreadis"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "4995",
      "title": "Speed up your development practices: Decouple application from infrastructure",
      "speakers": [
        {
          "fullName": "Mauricio Salatino"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "6551",
      "title": "Observability 101 with Spring and Micrometer",
      "speakers": [
        {
          "fullName": "Nele Uhlemann"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "6556",
      "title": "Faster, greener, and happier- why Quarkus should be your next tech stack",
      "speakers": [
        {
          "fullName": "Holly Cummins"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "6560",
      "title": "State of change detection in Angular and how far are we from Zoneless applications ? ",
      "speakers": [
        {
          "fullName": "Matthieu Riegler"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "6570",
      "title": "Hitting the “refresh” button on your Spring web application",
      "speakers": [
        {
          "fullName": "Stéphane Nicoll"
        },
        {
          "fullName": "Brian Clozel"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "6575",
      "title": "Avoid mastering Kubernetes for local development with Dapr and Testcontainers.",
      "speakers": [
        {
          "fullName": "Mauricio Salatino"
        },
        {
          "fullName": "Oleg Šelajev"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "7156",
      "title": "SQL Test Kit, or how to unit test any SQL query",
      "speakers": [
        {
          "fullName": "Victor Landeau"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "7163",
      "title": "From k9s to OpenTelemetry: A guide to observability for your apps in K8s",
      "speakers": [
        {
          "fullName": "Matthias Haeussler"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Room 7"
      }
    },
    {
      "id": "7181",
      "title": "Principles of Fluent API Design",
      "speakers": [
        {
          "fullName": "David Beaumont"
        }
      ],
      "format": {
        "duration": "PT20m",
        "id": "975",
        "title": "Byte Size"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "7182",
      "title": "The dark side of kotlin",
      "speakers": [
        {
          "fullName": "Jarek Ratajski"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    },
    {
      "id": "7190",
      "title": "React Server Components - Explained for Backend Developers",
      "speakers": [
        {
          "fullName": "Jonas Bandi"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Room 2"
      }
    },
    {
      "id": "7666",
      "title": "Using AI in Software Design: How ChatGPT Can Help With Creating a Solution Architecture",
      "speakers": [
        {
          "fullName": "Ralf D. Müller"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "9351",
        "title": "Room 8"
      }
    },
    {
      "id": "7670",
      "title": "Performance oriented Spring Data JPA & Hibernate",
      "speakers": [
        {
          "fullName": "Maciej Walkowiak"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "953",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Room 3"
      }
    }
  ]
} as const satisfies TalkMatchingYoutubeTestData;
  