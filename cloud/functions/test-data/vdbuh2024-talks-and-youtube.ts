
import {TalkMatchingYoutubeTestData} from "./test-data";

export const VDBUH2024_TALKS_AND_YOUTUBE = {
  "expectedMappedTalks": [
    {
      "__score": 0.581,
      "__talkTitle": "Scaling personalization with AI: leveraging HuggingFace and RAG for customized user experiences",
      "__videoTitle": "[VDBUH2024] - Nicolas Boitout & Bogdan Solga - Scaling personalization with AI",
      "__speakers": "Bogdan Solga, Nicolas Boitout",
      "talkId": "12902",
      "videoId": "JbNjzclyXh0"
    },
    {
      "__score": 0.679,
      "__talkTitle": "The Future is 💚: A Journey through the Energy Efficient Design Principles of Software Engineering",
      "__videoTitle": "[VDBUH2024]  - Ionuț Baloșin & Ko Turk - The Future is 💚: A Journey through the Energy Efficient ..",
      "__speakers": "Ko Turk, Ionut Balosin",
      "talkId": "2809",
      "videoId": "y7ZJ-XpdjNg"
    },
    {
      "__score": 0.767,
      "__talkTitle": "Innovation takes courage",
      "__videoTitle": "[VDBUH2024] - Alex Pavăl - Innovation takes courage",
      "__speakers": "Alex Paval",
      "talkId": "12306",
      "videoId": "ykJq6dXwNI4"
    },
    {
      "__score": 0.771,
      "__talkTitle": "Reading code",
      "__videoTitle": "[VDBUH2024] - Marit van Dijk - Reading code",
      "__speakers": "Marit van Dijk",
      "talkId": "1753",
      "videoId": "kKQqa27NiHM"
    },
    {
      "__score": 0.81,
      "__talkTitle": "Bootiful Spring Boot 3.x",
      "__videoTitle": "[VDBUH2024] - Josh Long - Bootiful Spring Boot 3.x",
      "__speakers": "Josh Long",
      "talkId": "11752",
      "videoId": "3SiYK0BWr0M"
    },
    {
      "__score": 0.833,
      "__talkTitle": "Code Smells - Hall of Fame",
      "__videoTitle": "[VDBUH2024] - Victor Rentea - Code Smells - Hall of Fame",
      "__speakers": "Victor Rentea",
      "talkId": "12305",
      "videoId": "lsW0-sEGr3w"
    },
    {
      "__score": 0.857,
      "__talkTitle": "A Practical Use Case of Software Architecture Evolution",
      "__videoTitle": "[VDBUH2024] - Ionuț Baloșin - A Practical Use Case of Software Architecture Evolution",
      "__speakers": "Ionut Balosin",
      "talkId": "5550",
      "videoId": "Yo1tNeaDQB0"
    },
    {
      "__score": 0.864,
      "__talkTitle": "Lessons learned as a DevOps Consultant",
      "__videoTitle": "[VDBUH2024] Mircea Rugină - Lessons Learned as a DevOps Consultant",
      "__speakers": "Mircea Rugina",
      "talkId": "12901",
      "videoId": "CS6K3dxyc0I"
    },
    {
      "__score": 0.873,
      "__talkTitle": "AI Showdown: The Good, The Glitchy, and The Risky",
      "__videoTitle": "[VDBUH2024] - Olimpiu Pop & Steve Poole - AI Showdown: The Good, The Glitchy, and The Risky",
      "__speakers": "Olimpiu Pop, Steve Poole",
      "talkId": "7456",
      "videoId": "WAxBGqiWCI0"
    },
    {
      "__score": 0.878,
      "__talkTitle": "Maximizing Security with JDK: Understanding Built-in Capabilities",
      "__videoTitle": "[VDBUH2024] - Laurențiu Spilcă - Maximizing Security with JDK: Understanding Built-in Capabilities",
      "__speakers": "Laurentiu Spilca",
      "talkId": "2801",
      "videoId": "TcCkSbari7M"
    },
    {
      "__score": 0.879,
      "__talkTitle": "Securing Your Java Containers by Breaking In",
      "__videoTitle": "[VDBUH2024] - Brian Vermeer - Securing Your Java Containers by Breaking In",
      "__speakers": "Brian Vermeer",
      "talkId": "4952",
      "videoId": "JKuc3tdzfUM"
    },
    {
      "__score": 0.88,
      "__talkTitle": "Performance and Scale Domain Oriented Objects vs Tabular Data Structures",
      "__videoTitle": "[VDBUH2024] Rustam Mehmandarov -Performance&Scale Domain Oriented Objects vs Tabular Data Structures",
      "__speakers": "Rustam Mehmandarov",
      "talkId": "11751",
      "videoId": "6NaIlETJ3Nw"
    },
    {
      "__score": 0.883,
      "__talkTitle": "Finding the balance between human and bot interaction",
      "__videoTitle": "[VDBUH2024] Andreea Mihăilescu&Alexandru Ivan -Finding the balance between human and bot interaction",
      "__speakers": "Andreea Mihailescu, Alexandru Ivan",
      "talkId": "12301",
      "videoId": "SqOZ6I4slok"
    },
    {
      "__score": 0.884,
      "__talkTitle": "CQRS in the small with Java Records and jOOQ",
      "__videoTitle": "[VDBUH2024] - Simon Martinelli - CQRS in the small with Java Records and jOOQ",
      "__speakers": "Simon Martinelli",
      "talkId": "3308",
      "videoId": "JVX8XKPg4xI"
    },
    {
      "__score": 0.889,
      "__talkTitle": "Cloud Cost Optimization for Java Developers",
      "__videoTitle": "[VDBUH2024] Pratik Patel - Cloud Cost Optimization for Java Developers",
      "__speakers": "PRATIK PATEL",
      "talkId": "11755",
      "videoId": "h4f-FunJ-w0"
    },
    {
      "__score": 0.893,
      "__talkTitle": "Beyond the Hammer: Diversifying Your Software Development Toolkit",
      "__videoTitle": "[VDBUH2024] - Liviu Spiroiu - Beyond the Hammer: Diversifying Your Software Development Toolkit",
      "__speakers": "Liviu N. Spiroiu",
      "talkId": "11756",
      "videoId": "uYSpY0m_3P0"
    },
    {
      "__score": 0.896,
      "__talkTitle": "Effecting Change—The Art of Leading Teams",
      "__videoTitle": "[VDBUH2024] Venkat Subramaniam - Effecting Change—The Art of Leading Teams",
      "__speakers": "Venkat Subramaniam",
      "talkId": "11754",
      "videoId": "7S9bepfGOrw"
    },
    {
      "__score": 0.898,
      "__talkTitle": "Monitoring Java Application Security with JDK tools and JFR Events",
      "__videoTitle": "[VDBUH2024] Ana-Maria Mihălceanu -Monitoring Java Application Security with JDK tools and JFR Events",
      "__speakers": "Ana-Maria Mihalceanu",
      "talkId": "7801",
      "videoId": "tAtSDsU9Hao"
    },
    {
      "__score": 0.899,
      "__talkTitle": "Continuous Profiling, the missing piece in your observability puzzle!",
      "__videoTitle": "[VDBUH2024] Mohammed Aboullaite -Continuous Profiling the missing piece in your observability puzzle",
      "__speakers": "Mohammed Aboullaite",
      "talkId": "1754",
      "videoId": "HSW-h3Ipi3Y"
    },
    {
      "__score": 0.906,
      "__talkTitle": "From Spring Boot 2 to Spring Boot 3 with Java 21 and Jakarta EE",
      "__videoTitle": "[VDBUH2024] - Ivar Grimstad - From Spring Boot 2 to Spring Boot 3 with Java 21 and Jakarta EE",
      "__speakers": "Ivar Grimstad",
      "talkId": "5512",
      "videoId": "CfTjir73BzQ"
    },
    {
      "__score": 0.908,
      "__talkTitle": "Five things every developer should know about software architecture",
      "__videoTitle": "[VDBUH2024] - Simon Brown - Five things every developer should know about software architecture",
      "__speakers": "Simon Brown",
      "talkId": "11753",
      "videoId": "wcrloUXSFgY"
    },
    {
      "__score": 0.91,
      "__talkTitle": "Practical Guides for Enhancing Your Software Supply Chain Security",
      "__videoTitle": "[VDBUH2024] - Abdel Sghiouar - Practical Guides for Enhancing Your Software Supply Chain Security",
      "__speakers": "Abdel Sghiouar",
      "talkId": "3907",
      "videoId": "cn-puSzSiRo"
    },
    {
      "__score": 0.911,
      "__talkTitle": "How sand and Java are used to create the world’s most powerful chips",
      "__videoTitle": "[VDBUH2024] - Johan Janssen - How sand and Java are used to create the world’s most powerful chips",
      "__speakers": "Johan Janssen",
      "talkId": "4351",
      "videoId": "tRd6uQdL8os"
    },
    {
      "__score": 0.911,
      "__talkTitle": "AI is Coming - Now What? Architecting the Future of Software",
      "__videoTitle": "[VDBUH2024] - Hugh Mckee - AI is Coming - Now What? Architecting the Future of Software",
      "__speakers": "Hugh McKee",
      "talkId": "5548",
      "videoId": "I9S6k4WgB6k"
    }
  ],
  "expectedUnmappedTalks": [
    {
      "__talkTitle": "PANEL: Mastering the AI Shift: Adaptive Strategies for Software Evolution",
      "__talkFormat": "Panel (id=989, duration=PT45m)",
      "__talkRoom": "Crowne B (4302)",
      "__talkSpeakers": "Lucian Gruia, Brian Vermeer, Olimpiu Pop, valentin Tomici, PRATIK PATEL, Cristina Duta, Mihaela CUȚUI",
      "talkId": "12602"
    },
    {
      "__talkTitle": " Fast And Lightweight Spring Boot Applications With GraalVM",
      "__talkFormat": "Conference (id=951, duration=PT60m)",
      "__talkRoom": "Crowne A (4301)",
      "__talkSpeakers": "Alina Yurenko",
      "talkId": "1761"
    },
    {
      "__talkTitle": "Coding Context-Aware RAG Applications for Enterprise-Level",
      "__talkFormat": "Hands-on Lab (id=984, duration=PT120m)",
      "__talkRoom": "Crown D (12102)",
      "__talkSpeakers": "Lucian Gruia",
      "talkId": "4651"
    },
    {
      "__talkTitle": "Cloud Native Java on Kubernetes",
      "__talkFormat": "Hands-on Lab (id=984, duration=PT120m)",
      "__talkRoom": "Crown D (12102)",
      "__talkSpeakers": "Andra Somesan, Adrian Bere",
      "talkId": "5531"
    }
  ],
  "youtubeVideos": [
    {
      "id": "OTcNjWA0RZM",
      "publishedAt": "2024-04-18T09:08:40Z",
      "duration": "PT48M10S",
      "title": "[VDBUH2024] - Mastering the AI Shift: Adaptive Strategies for Software Evolution"
    },
    {
      "id": "tXJgJsd_xsQ",
      "publishedAt": "2024-04-18T04:17:26Z",
      "duration": "PT5M10S",
      "title": "[VDBUH2024] -  Alex Proca & Andra Ghibuțiu - Opening Thoughts"
    },
    {
      "id": "uYSpY0m_3P0",
      "publishedAt": "2024-04-18T04:17:26Z",
      "duration": "PT48M22S",
      "title": "[VDBUH2024] - Liviu Spiroiu - Beyond the Hammer: Diversifying Your Software Development Toolkit"
    },
    {
      "id": "wcrloUXSFgY",
      "publishedAt": "2024-04-18T04:17:26Z",
      "duration": "PT44M50S",
      "title": "[VDBUH2024] - Simon Brown - Five things every developer should know about software architecture"
    },
    {
      "id": "y7ZJ-XpdjNg",
      "publishedAt": "2024-04-18T04:17:26Z",
      "duration": "PT48M51S",
      "title": "[VDBUH2024]  - Ionuț Baloșin & Ko Turk - The Future is 💚: A Journey through the Energy Efficient .."
    },
    {
      "id": "ykJq6dXwNI4",
      "publishedAt": "2024-04-18T04:17:26Z",
      "duration": "PT41M7S",
      "title": "[VDBUH2024] - Alex Pavăl - Innovation takes courage"
    },
    {
      "id": "SqOZ6I4slok",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT44M48S",
      "title": "[VDBUH2024] Andreea Mihăilescu&Alexandru Ivan -Finding the balance between human and bot interaction"
    },
    {
      "id": "TcCkSbari7M",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT40M9S",
      "title": "[VDBUH2024] - Laurențiu Spilcă - Maximizing Security with JDK: Understanding Built-in Capabilities"
    },
    {
      "id": "WAxBGqiWCI0",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT54M33S",
      "title": "[VDBUH2024] - Olimpiu Pop & Steve Poole - AI Showdown: The Good, The Glitchy, and The Risky"
    },
    {
      "id": "Yo1tNeaDQB0",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT49M46S",
      "title": "[VDBUH2024] - Ionuț Baloșin - A Practical Use Case of Software Architecture Evolution"
    },
    {
      "id": "cn-puSzSiRo",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT34M31S",
      "title": "[VDBUH2024] - Abdel Sghiouar - Practical Guides for Enhancing Your Software Supply Chain Security"
    },
    {
      "id": "h4f-FunJ-w0",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT1H2M41S",
      "title": "[VDBUH2024] Pratik Patel - Cloud Cost Optimization for Java Developers"
    },
    {
      "id": "kKQqa27NiHM",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT48M56S",
      "title": "[VDBUH2024] - Marit van Dijk - Reading code"
    },
    {
      "id": "lsW0-sEGr3w",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT45M42S",
      "title": "[VDBUH2024] - Victor Rentea - Code Smells - Hall of Fame"
    },
    {
      "id": "tAtSDsU9Hao",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT51M57S",
      "title": "[VDBUH2024] Ana-Maria Mihălceanu -Monitoring Java Application Security with JDK tools and JFR Events"
    },
    {
      "id": "tRd6uQdL8os",
      "publishedAt": "2024-04-18T04:17:24Z",
      "duration": "PT39M14S",
      "title": "[VDBUH2024] - Johan Janssen - How sand and Java are used to create the world’s most powerful chips"
    },
    {
      "id": "6NaIlETJ3Nw",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT1H1M8S",
      "title": "[VDBUH2024] Rustam Mehmandarov -Performance&Scale Domain Oriented Objects vs Tabular Data Structures"
    },
    {
      "id": "7S9bepfGOrw",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT50M30S",
      "title": "[VDBUH2024] Venkat Subramaniam - Effecting Change—The Art of Leading Teams"
    },
    {
      "id": "CS6K3dxyc0I",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT41M36S",
      "title": "[VDBUH2024] Mircea Rugină - Lessons Learned as a DevOps Consultant"
    },
    {
      "id": "CfTjir73BzQ",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT54M36S",
      "title": "[VDBUH2024] - Ivar Grimstad - From Spring Boot 2 to Spring Boot 3 with Java 21 and Jakarta EE"
    },
    {
      "id": "I9S6k4WgB6k",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT54M11S",
      "title": "[VDBUH2024] - Hugh Mckee - AI is Coming - Now What? Architecting the Future of Software"
    },
    {
      "id": "JKuc3tdzfUM",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT52M15S",
      "title": "[VDBUH2024] - Brian Vermeer - Securing Your Java Containers by Breaking In"
    },
    {
      "id": "JVX8XKPg4xI",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT47M36S",
      "title": "[VDBUH2024] - Simon Martinelli - CQRS in the small with Java Records and jOOQ"
    },
    {
      "id": "JbNjzclyXh0",
      "publishedAt": "2024-04-18T04:17:22Z",
      "duration": "PT1H5M30S",
      "title": "[VDBUH2024] - Nicolas Boitout & Bogdan Solga - Scaling personalization with AI"
    },
    {
      "id": "3SiYK0BWr0M",
      "publishedAt": "2024-04-18T04:17:21Z",
      "duration": "PT58M10S",
      "title": "[VDBUH2024] - Josh Long - Bootiful Spring Boot 3.x"
    },
    {
      "id": "HSW-h3Ipi3Y",
      "publishedAt": "2024-04-18T04:17:21Z",
      "duration": "PT55M48S",
      "title": "[VDBUH2024] Mohammed Aboullaite -Continuous Profiling the missing piece in your observability puzzle"
    }
  ],
  "talks": [
    {
      "id": "11751",
      "title": "Performance and Scale Domain Oriented Objects vs Tabular Data Structures",
      "speakers": [
        {
          "fullName": "Rustam Mehmandarov"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "11752",
      "title": "Bootiful Spring Boot 3.x",
      "speakers": [
        {
          "fullName": "Josh Long"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "969",
        "title": "Keynote"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "11753",
      "title": "Five things every developer should know about software architecture",
      "speakers": [
        {
          "fullName": "Simon Brown"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "956",
        "title": "Keynote"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "11754",
      "title": "Effecting Change—The Art of Leading Teams",
      "speakers": [
        {
          "fullName": "Venkat Subramaniam"
        }
      ],
      "format": {
        "duration": "PT55m",
        "id": "12052",
        "title": "Keynote "
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "11755",
      "title": "Cloud Cost Optimization for Java Developers",
      "speakers": [
        {
          "fullName": "PRATIK PATEL"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "969",
        "title": "Keynote"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "11756",
      "title": "Beyond the Hammer: Diversifying Your Software Development Toolkit",
      "speakers": [
        {
          "fullName": "Liviu N. Spiroiu"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Crowne C"
      }
    },
    {
      "id": "12301",
      "title": "Finding the balance between human and bot interaction",
      "speakers": [
        {
          "fullName": "Andreea Mihailescu"
        },
        {
          "fullName": "Alexandru Ivan"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "12305",
      "title": "Code Smells - Hall of Fame",
      "speakers": [
        {
          "fullName": "Victor Rentea"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "956",
        "title": "Keynote"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "12306",
      "title": "Innovation takes courage",
      "speakers": [
        {
          "fullName": "Alex Paval"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "12602",
      "title": "PANEL: Mastering the AI Shift: Adaptive Strategies for Software Evolution",
      "speakers": [
        {
          "fullName": "Lucian Gruia"
        },
        {
          "fullName": "Brian Vermeer"
        },
        {
          "fullName": "Olimpiu Pop"
        },
        {
          "fullName": "valentin Tomici"
        },
        {
          "fullName": "PRATIK PATEL"
        },
        {
          "fullName": "Cristina Duta"
        },
        {
          "fullName": "Mihaela CUȚUI"
        }
      ],
      "format": {
        "duration": "PT45m",
        "id": "989",
        "title": "Panel"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "12901",
      "title": "Lessons learned as a DevOps Consultant",
      "speakers": [
        {
          "fullName": "Mircea Rugina"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "12902",
      "title": "Scaling personalization with AI: leveraging HuggingFace and RAG for customized user experiences",
      "speakers": [
        {
          "fullName": "Bogdan Solga"
        },
        {
          "fullName": "Nicolas Boitout"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Crowne C"
      }
    },
    {
      "id": "1753",
      "title": "Reading code",
      "speakers": [
        {
          "fullName": "Marit van Dijk"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Crowne C"
      }
    },
    {
      "id": "1754",
      "title": "Continuous Profiling, the missing piece in your observability puzzle!",
      "speakers": [
        {
          "fullName": "Mohammed Aboullaite"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4303",
        "title": "Crowne C"
      }
    },
    {
      "id": "1761",
      "title": " Fast And Lightweight Spring Boot Applications With GraalVM",
      "speakers": [
        {
          "fullName": "Alina Yurenko"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "2801",
      "title": "Maximizing Security with JDK: Understanding Built-in Capabilities",
      "speakers": [
        {
          "fullName": "Laurentiu Spilca"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "2809",
      "title": "The Future is 💚: A Journey through the Energy Efficient Design Principles of Software Engineering",
      "speakers": [
        {
          "fullName": "Ko Turk"
        },
        {
          "fullName": "Ionut Balosin"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "3308",
      "title": "CQRS in the small with Java Records and jOOQ",
      "speakers": [
        {
          "fullName": "Simon Martinelli"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "3907",
      "title": "Practical Guides for Enhancing Your Software Supply Chain Security",
      "speakers": [
        {
          "fullName": "Abdel Sghiouar"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "4351",
      "title": "How sand and Java are used to create the world’s most powerful chips",
      "speakers": [
        {
          "fullName": "Johan Janssen"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "4651",
      "title": "Coding Context-Aware RAG Applications for Enterprise-Level",
      "speakers": [
        {
          "fullName": "Lucian Gruia"
        }
      ],
      "format": {
        "duration": "PT120m",
        "id": "984",
        "title": "Hands-on Lab"
      },
      "room": {
        "id": "12102",
        "title": "Crown D"
      }
    },
    {
      "id": "4952",
      "title": "Securing Your Java Containers by Breaking In",
      "speakers": [
        {
          "fullName": "Brian Vermeer"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "5512",
      "title": "From Spring Boot 2 to Spring Boot 3 with Java 21 and Jakarta EE",
      "speakers": [
        {
          "fullName": "Ivar Grimstad"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    },
    {
      "id": "5531",
      "title": "Cloud Native Java on Kubernetes",
      "speakers": [
        {
          "fullName": "Andra Somesan"
        },
        {
          "fullName": "Adrian Bere"
        }
      ],
      "format": {
        "duration": "PT120m",
        "id": "984",
        "title": "Hands-on Lab"
      },
      "room": {
        "id": "12102",
        "title": "Crown D"
      }
    },
    {
      "id": "5548",
      "title": "AI is Coming - Now What? Architecting the Future of Software",
      "speakers": [
        {
          "fullName": "Hugh McKee"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "5550",
      "title": "A Practical Use Case of Software Architecture Evolution",
      "speakers": [
        {
          "fullName": "Ionut Balosin"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "7456",
      "title": "AI Showdown: The Good, The Glitchy, and The Risky",
      "speakers": [
        {
          "fullName": "Olimpiu Pop"
        },
        {
          "fullName": "Steve Poole"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4302",
        "title": "Crowne B"
      }
    },
    {
      "id": "7801",
      "title": "Monitoring Java Application Security with JDK tools and JFR Events",
      "speakers": [
        {
          "fullName": "Ana-Maria Mihalceanu"
        }
      ],
      "format": {
        "duration": "PT60m",
        "id": "951",
        "title": "Conference"
      },
      "room": {
        "id": "4301",
        "title": "Crowne A"
      }
    }
  ]
} as const satisfies TalkMatchingYoutubeTestData;
  