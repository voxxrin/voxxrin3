...
beforeAll(async () => {
    await Promise.all([
        ...
        adminFirestore.doc('/events/an-event/roomsStats-allInOne/self').set({ "12345": { id: `12345`, capacityFillingRatio: 0, recordedAt: "2024-03-28T11:58:10Z", persistedAt: "2024-03-28T12:00:00Z" } }),
...
const COLLECTIONS: CollectionDescriptor[] = [{
    ...
    tests: (userContext: UserContext) => {
        ...
        it(`As ${userContext.name}, I should not be able to CREATE events' all-in-one rooms stats`, async () => {
            await assertFails(setDoc(doc(userContext.context().firestore(), '/events/another-event/roomsStats-allInOne/self'), { "12345": { id: `12345`, capacityFillingRatio: 0, recordedAt: "2024-03-28T11:58:10Z", persistedAt: "2024-03-28T12:00:00Z" } }));
        })
        it(`As ${userContext.name}, I should not be able to UPDATE events' all-in-one rooms stats`, async () => {
            await assertFails(updateDoc(doc(userContext.context().firestore(), '/events/an-event/roomsStats-allInOne/self'), { "12345": { id: `12345`, capacityFillingRatio: 1, recordedAt: "2024-03-28T11:58:10Z", persistedAt: "2024-03-28T12:00:00Z" } }));
        })
